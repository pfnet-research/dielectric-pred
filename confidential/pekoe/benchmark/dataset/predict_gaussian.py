...
def main(args: argparse.Namespace) -> None:
    estimator = build_estimator(
        device=args.gpu, path_lib="pfp.nn.models.crystal.lib.libinfer"
    )
    ...
    data_name_list = [
        ...
        [
            ...
            "pubchem_CNOPS7_rad_nms",
            "pubchem_CNOPS7_FClBr_opt",
            "pubchem_CNOPS7_FClBr_nms",
        ],
        [
            "pubchem_CNOPS8_opt",
            "pubchem_CNOPS8_nms",
    ...
    test_dataset = h5py.File("test_" + args.data_path, "r")
    train_dataset = h5py.File(args.data_path, "r")

    pdf_pages = PdfPages("test_molecule_energy_force_debug.pdf")
    ...
    for i_list, data_names in enumerate(data_name_list):
        dataset_name_arr = [
            ...
            ("rad_nms", data_names[4]),
            ("halide_opt", data_names[5]),
        ]
        if len(data_names) >= 7:
            dataset_name_arr.append(("halide_nms", data_names[6]))
        ...
        for dataset_name, dataset_key in dataset_name_arr:
            if dataset_name == "dts":
                ...
                merged_dataset = test_dataset

            dataset = merged_dataset[dataset_key]
            data = []
            ...
            for i, (sample_key, sample_idx) in enumerate(
                ...
                sample = merged_dataset[sample_key]
                data.append(
                    {
