...
def ssp(x: torch.Tensor) -> torch.Tensor:
    ...
    """
    return x * torch.log1p(F.softplus(x))


def ssp_grad(x: torch.Tensor) -> torch.Tensor:
    """Grad of ssp"""
    y: torch.Tensor = torch.log1p(F.softplus(x)) + x / (1.0 + torch.exp(-x)) / (
        1.0 + F.softplus(x)
    )
    return y


def ssp_even(x: torch.Tensor) -> torch.Tensor:
    return ssp(x) + ssp(-x)
...
def logcosh(x: torch.Tensor) -> torch.Tensor:
    # TODO(takamoto) should be replaced to torch.abs later. (abs backword issue in PFVM)
    absx = torch.max(-x, x)
    return (
        absx
        + torch.nn.functional.softplus(-2.0 * absx)
        - torch.log(torch.tensor(2.0, dtype=x.dtype, device=x.device))
    )
